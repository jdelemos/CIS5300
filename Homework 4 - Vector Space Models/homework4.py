...
Original file is located at
    https://colab.research.google.com/drive/1p8WQ8PWzBrPvKWw6MfcGxTNW7xRgMYeX
...
def create_term_document_matrix(line_tuples, document_names, vocab):
	...
	'''
	print(line_tuples[0][0])
	vocab_to_id = dict(zip(vocab, range(0, len(vocab))))
	print(vocab_to_id)
	docname_to_id = dict(zip(document_names, range(0, len(document_names))))
	print(docname_to_id)
	final_matrix = np.zeros((len(vocab_to_id), len(docname_to_id)))
  # looped too many times
	# for x in range(len(final_matrix)):
	# 	for y in range(len(final_matrix[0])):
	# 		for key_scene,value_scene in docname_to_id.items():
	# 			for key_word,value_word in vocab_to_id.items():
	# 				for line in line_tuples[value_scene]:
	# 					for word in line:
	# 						if word == key_word:
	# 							final_matrix[x][y] = final_matrix[x][y] + 1
	for doc_name, line in line_tuples:
		# [henry_IV, '0'], this grabs the numerical position of the play
		num = docname_to_id[doc_name]
		# now we are grabbing the actual word
		for word in line:
			final_matrix[[vocab_to_id[word]], num] +=1
...
def compute_cosine_similarity(vector1, vector2):
	...
	if denom == 0:
			return 0.0  # define similarity with a zero vector as 0
...
def create_term_context_matrix(line_tuples, vocab, context_window_size):
    vocab_to_id = {w: i for i, w in enumerate(vocab)}
    n = len(vocab)
    final_matrix = np.zeros((n, n))

    for _, line in line_tuples:
			  #enumerate is an excellent option for trying
        for pos, word in enumerate(line):
            if word not in vocab_to_id:
                continue
            i = vocab_to_id[word]
            # context positions
            left = max(0, pos - context_window_size)
            right = min(len(line), pos + context_window_size + 1)
            for cpos in range(left, right):
                if cpos == pos:
                    continue
                context_word = line[cpos]
                if context_word in vocab_to_id:
                    j = vocab_to_id[context_word]
                    final_matrix[i, j] += 1
    return final_matrix
...
def create_PPMI_matrix(term_context_matrix):
    rows, cols = term_context_matrix.shape
    epsilon = 1e-6

    # Apply epsilon early
    matrix = term_context_matrix + epsilon
    matrix_summation = np.sum(matrix)

    # Joint probabilities
    joint_prob_matrix = matrix / matrix_summation

    # Marginals
    row_marginal = np.sum(matrix, axis=1, keepdims=True) / matrix_summation
    col_marginal = np.sum(matrix, axis=0, keepdims=True) / matrix_summation

    # PMI + clamp (vectorized instead of nested loops)
    expected = row_marginal @ col_marginal
    mask = (joint_prob_matrix > 0) & (expected > 0)

    pmi = np.zeros_like(joint_prob_matrix)
    pmi[mask] = np.log2(joint_prob_matrix[mask] / expected[mask])
...
def create_tf_idf_matrix(term_document_matrix):
	...
	# df = number of documents the word appeared in.
	# N = number of documents
	# idf = log_10(N/df(i))
	rows, cols = term_document_matrix.shape
	df = np.zeros(rows)
	for i in range(rows):
		count = 0
		for j in range(cols):
	...
	idf = np.zeros(rows)
	for i in range(rows):
		idf[i] = np.log10(cols/(df[i]+1e-6))
	tf = term_document_matrix

	# YOUR CODE HERE
	tf_idf = np.zeros((rows, cols))
	for i in range(rows):
			for j in range(cols):
					tf_idf[i, j] = term_document_matrix[i, j] * idf[i]
...
def compute_dice_similarity(vector1, vector2):
	...
	# YOUR CODE HERE
	return 2 * np.sum(np.minimum(vector1, vector2)) / (np.sum(vector1) + np.sum(vector2))
...
def rank_plays(target_play_index, term_document_matrix, similarity_fn):
	...
	sims = np.zeros(n)
	for i in range(n):
		sims[i] = similarity_fn(term_document_matrix[:, target_play_index], term_document_matrix[:, i])
	sims_sort = np.argsort(sims)[::-1]
	return sims_sort
...
def rank_words(target_word_index, matrix, similarity_fn):
	...
	for i in range(matrix.shape[0]):
		sims[i] = similarity_fn(matrix[target_word_index], matrix[i])
	sims_sort = np.argsort(sims)[::-1]
	return sims_sort
...
with open(file_path, 'r', newline='') as csvfile:
    ...
    for row in reader:
        if len(row) > 5:
            char = row[4].strip()
            line = re.sub(r'[^a-zA-Z0-9\s]', ' ', row[5]).split()
            tokens = [t.lower() for t in line]
            char_lines[char].extend(tokens)

char_tuples = [(char, tokens) for char, tokens in char_lines.items() if len(tokens) > 500]



def create_character_term_matrix(char_tuples, vocab):
    vocab_to_id = {w: i for i, w in enumerate(vocab)}
    char_names = sorted({c for c, _ in char_tuples})
    char_to_id = {c: i for i, c in enumerate(char_names)}

    matrix = np.zeros((len(char_names), len(vocab)))

    for char, tokens in char_tuples:
        if char not in char_to_id:
            continue
        row = char_to_id[char]
        for word in tokens:
            if word in vocab_to_id:
                col = vocab_to_id[word]
                matrix[row, col] += 1
    return matrix, char_names

matrix, char_names = create_character_term_matrix(char_tuples, vocab)
tmp_PPMI_matrix = create_tf_idf_matrix(matrix)
# matrix, char_names = create_character_term_matrix(char_tuples, vocab)
# print(char_names)
target_word = 'MACBETH'
i = char_names.index(target_word)

sims = np.zeros(matrix.shape[0])
for j in range(matrix.shape[0]):
    sims[j] = compute_cosine_similarity(tmp_PPMI_matrix[i], tmp_PPMI_matrix[j])

ranking = np.argsort(sims)[::-1]

for idx in ranking[1:6]:
    print(char_names[idx], sims[idx])

print(ranking)
# WRONG DIRECTION
# NUM = 0
# char = ''
# unique_chars = sorted(set(char_names))
# for i,w in enumerate(unique_chars):

#   if w == 'MACBETH':
#     char = w
#     NUM = i

# main_char = [char] * len(char_names)
# tuples_comp = zip(main_char, unique_chars )
# tuples_ = list(tuples_comp)

# N = 6000

# tmp_vocab = vocab[:N] # we only use part of the vocab so that you won't blow up the RAM
# term_context_matrix = create_term_context_matrix(tuples_, tmp_vocab, context_window_size = 2)
# tmp_PPMI_matrix = create_PPMI_matrix(term_context_matrix)
...
# target_word_ranking = rank_words(i, tmp_PPMI_matrix, compute_cosine_similarity)
# print(target_word_ranking)

#print(char_names)
# main_char = [char_names[0]] * len(char_names)
# print(char_names)
# tuples_comp = zip(main_char, char_names)
# tuples_ = list(tuples_comp)
# print(tuples_[:10])
...
results = []
for window in [3,4,5]:
    context_data = create_term_context_matrix(tuples, vocab, context_window_size=window)
    PPMI_matrix = create_PPMI_matrix(context_data)
    print("Window", window, "matrix sum:", context_data.sum())
    model_scores = []
    human_scores = []
    model_scores2 = []
    human_scores2 = []

    for w1, w2, sc in zip(word1, word2, score):
        if w1 in vocab_index and w2 in vocab_index:

            v1 = PPMI_matrix[vocab_index[w1]]
            v2 = PPMI_matrix[vocab_index[w2]]
            sim = compute_cosine_similarity(v1, v2)
            mis = compute_dice_similarity(v1, v2)
            model_scores.append(sim)
            human_scores.append(sc)
            model_scores2.append(mis)
            human_scores2.append(sc)

    print("Valid pairs used:", len(human_scores))
    tau, p = kendalltau(human_scores, model_scores)
    tau1, p1 = kendalltau(human_scores2, model_scores2)
    results.append((window, tau, p))
    results2.append((window,tau1,p1))
